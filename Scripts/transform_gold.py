...
SCORE_PONDERE_WEIGHT = 0.70
PIRE_FACTEUR_WEIGHT = 0.30
...
def find_latest_run_ts():
    ...
    logger.info(f"Run Silver le plus récent trouvé : {dernier_run}")
    return dernier_run
...
def load_silver(run_ts):
    ...
    logger.info(f"Prévisions Silver chargées : {len(df)}")
    return df
...
def compute_risk_score(row):
    ...
    pire_facteur = max(temperature_risk, precipitation_risk, wind_risk)

    score = (
        score_pondere * SCORE_PONDERE_WEIGHT
        + pire_facteur * PIRE_FACTEUR_WEIGHT
    )
    return round(score)
...
def save_gold(df, run_ts):
    ...
    logger.info(f"Fichier Gold sauvegardé : {chemin_sortie}")
    return chemin_sortie


def main():
    run_ts = find_latest_run_ts()
    logger.info(f"=== Démarrage du traitement Gold pour le run {run_ts} ===")
    ...
    logger.info(f"=== Traitement Gold terminé pour le run {run_ts} ===")
